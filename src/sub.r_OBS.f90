...
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!############################################################################
!#  MODIFICATIONS vs sub.r_OBS_old.f90:                                     #
!############################################################################
!#                                                                          #
!#  [CHANGE] Use velocity waveforms directly (Line 58, 74-77)               #
!#                                                                          #
!#  Background: The old version called diff_wave() to numerically           #
!#  differentiate displacement waveforms to velocity, introducing a 0.5*dt0 #
!#  time shift that required compensation in shift and ndj calculations.    #
!#                                                                          #
!#  OLD (sub.r_OBS_old.f90 Line 58, 75-76):                                 #
!#    call diff_wave(wv,dt0,nd0)                                            #
!#    shift = shift + dt0*0.5                                               #
!#    ndj(j) = nint( (nd1*dt0 - p_sec + dt0*0.5) / dt(j) )                  #
!#                                                                          #
!#  NEW (this file):                                                        #
!#    ! call diff_wave(wv,dt0,nd0)   <-- COMMENTED OUT                      #
!#    shift = - p_sec                <-- Simplified, no dt0*0.5 correction  #
!#    ndj(j) = nint( (nd1*dt0 - p_sec) / dt(j) )  <-- Simplified            #
!#                                                                          #
!#  Requirement: Input waveforms in wave.obs/ should be VELOCITY (um/s),    #
!#  not displacement. This may avoid low-frequency drift issues             #
!#  and numerical differentiation artifacts.                                #
!#                                                                          #
!############################################################################
