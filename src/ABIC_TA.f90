Program ABIC_TA
   ...
   !
   !############################################################################
   !#  MODIFICATIONS vs ABIC_TA_old.f90:                                       #
   !############################################################################
   !#                                                                          #
   !#  [BUGFIX] ndj and cp array allocation and assignment (Line 61-64)        #
   !#                                                                          #
   !#  OLD (ABIC_TA_old.f90 Line 61-63):                                       #
   !#    allocate(..., ndj(j))           ! BUG: should be ndj(jn)              #
   !#    read(60,*) ..., ndj(j), cp(jn)  ! BUG: should be cp(j)                #
   !#                                                                          #
   !#  NEW (this file):                                                        #
   !#    allocate(..., ndj(jn))                                                #
   !#    read(60,*) ..., ndj(j), cp(j)                                         #
   !#                                                                          #
   !#  Impact: The old version would cause incorrect ndj array allocation and  #
   !#          cp values being assigned to wrong indices                       #
   !#                                                                          #
   !############################################################################
   implicit none
   ...
   do j=1,jn
      read(60,*) stcd(j),comp(j),dt(j),sigw(j),sigwd(j),ndj(j),cp(j)
   enddo
   close(60)
   ndmax = maxval(ndj)
   !+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
