...
!
!############################################################################
!#  MODIFICATIONS vs get_covariance_grn_old.f90:                            #
!############################################################################
!#                                                                          #
!#  [BUGFIX] OpenMP parallel computation                                    #
!#                                                                          #
!#  OLD (get_covariance_grn_old.f90 Line 56, 118, 123, 141, 151):           #
!#    real,allocatable:: ..., gb(:), ...                                    #
!#    !$omp parallel do private(..., gb, ...)                               #
!#    allocate(..., gb(imax))          ! Allocated inside parallel region   #
!#    call resample_shift(..., gb, ...)                                     #
!#    deallocate(..., gb)                                                   #
!#                                                                          #
!#  Problem: The variable 'gb' was already used and deallocated in the      #
!#  main program (Line 70, 87) before entering the parallel region.         #
!#  Re-using the same name 'gb' in OpenMP private clause caused conflicts.  #
!#                                                                          #
!#  NEW (this file):                                                        #
!#    real,allocatable:: ..., gb(:), gb_t(:), ...  ! Added gb_t             #
!#    !$omp parallel do private(..., gb_t, ...)    ! Use gb_t in parallel   #
!#    allocate(..., gb_t(imax))                    ! Allocate gb_t          #
!#    call resample_shift(..., gb_t, ...)          ! Use gb_t               #
!#    deallocate(..., gb_t)                        ! Deallocate gb_t        #
!#                                                                          #
!#  Impact: Fixes segmentation fault when running with OpenMP               #
...
subroutine covariance_grn(stcd,comp,ndj,dt,jn,tr,mn,nn,m0,n0,rtime,jtn0,icmn)
   ...
   !$omp parallel do  private(dtg,ndt,lkmax,Eg,gb_t,lk,icm,m,n,l,tw)
   do j=1,jn
      dtg  =  dt(j)/2.
      ndt = nint(ndj(j)*(dt(j)/dtg)) + 10
      lkmax = icmn * mn * nn * ndt + 1
      allocate(Eg(lkmax,ndj(j)),gb_t(imax))
      !-----------------------------
      ...
      do icm =1,icmn     ! icm -> q  in YF2011
         do m=1,mn;  do n=1,nn   ! space(m,n)  -> k  in YF2011
               do l=1,ndt
                  ...
                  call resample_shift(slip_rate(1,m,n,icm),imax,dts,gb_t,imax,dt(j),tw)
                  lk = lk + 1
                  eg(lk,1:ndj(j)) = gb_t(1:ndj(j)) * xmax(m,n,icm,j)   ! S_qkj*P_qkj(a) in YF2011
               enddo
            enddo; enddo
      enddo
